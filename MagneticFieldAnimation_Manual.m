...
%     phid_loc = phid(t_ind{ii}(L),ii); % phase of the local peak
%     phid_next = phid(t_ind{ii+1}((find(t_ind{ii+1}>t_ind{ii}(L),1))),ii+1); % phase of the next location
    ...
    if  ii<10 && sum(t_ind{ii+1}>t_ind{ii}(L))>=1 %...
        ...
        % There is a future peak in the next axial array and 
