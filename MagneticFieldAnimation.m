...
if LoadLocal == 0
    ...
    % Save workspace to a local file
    if SaveLocal == 1
        save(name);
    end
else
...
m1m0 = 0;
Camera = 0; % Synchronizes frame with the Shimazu camera
Contour = 0;
M1y = 0;
...
OnlyContour = 1; % Only plot contour on a separate figure
VisBeginning = 1; % Plot from the beginning of m1mo and IP (=1) or not
...
% =========================================================================
delay = 0.1; % Delay between each frame of the generated gif.
if Camera == 1
    % Time at which the camera took a snapshot
    t_camera = 54.99:0.1:80.5; % microsec, 141201018
...
% for i = 98:105
%     plot(time(vis_start:vis_end)*1e6, data{i}(vis_start:vis_end));
% end
timecursor_m1m0 = plot(NaN,NaN);
xlabel('Time [\mus]')
ylabel('B_1/B_0')
% legend('p5', 'p10', 'p15', 'p20', 'p25', 'p30', 'p35', 'p40', 'p45','Location','NorthWest')
pos = get(gca, 'Position');
pos(1) = pos(1);
pos(2) = ymargin*pos(2);
set(gca, 'Position', pos)
...
%         ylabel('')
    ...
    %     imwrite(imind,cmap,['gifs\',num2str(shotnum), '_m1m0.gif'],'gif','Loopcount',inf,'DelayTime',delay);
end

% % Raw magnetic contour
% ppp=figure(4);
    fig=gcf;
...
% 
% if OnlyContour ==1
%     onlyc= figure('units','normalized','outerposition',[0 0 1 0.6]);
%     aaa=gca;
%     [cont, hc] = contourf([], [], []);
%     xlabel('z [cm]')
%     ylabel('\theta [deg]')
%     caxis([-1 1])
%     range = -1:.1:1;
%     aaa.YTick = [0 45 90 135 180 225 270 315];
%     colorbar
%     colormap('winter')
% end
% Normalized magnetic data
% figure(5)
% [contq, hcq] = contour([], [], []);
% title('Normalized magnetic data at each axial location for each angle')
% xlabel('z [cm]2')
% ylabel('theta [rad]2')
% caxis([0.01 0.41])
% rangeq = 0.01:0.05:0.41;
% colorbar

%     frame=getframe(5);
%     im=frame2im(frame);
%     [imind,cmap]=rgb2ind(im,256);
%     imwrite(imind,cmap,['gifs\',num2str(shotnum), '_m1m0cont.gif'],'gif','Loopcount',inf,'DelayTime',delay);

k = 0;

% Magnetic field at p0 over time
% figure(2)
% i = 2; % p5
% plot(time(vis_start:vis_end), data{i}(vis_start:vis_end));
% hold on
% for i = 3:9
%     plot(time(vis_start:vis_end), data{i}(vis_start:vis_end));
% end
% xlabel('Time [s]')
% legend('0', '45', '90', '135', '180', '225', '270', '315', 'Location','NorthWest')
% title('magnetic field at p0 over time')

if VisBeginning ==1
    t_pos_start = vis_start;
    t_pos_end = vis_end;
else
    % Starting and ending time
    tstart = 62.239; % Starting time microsec
    tend = tstart+8*0.0401; % Ending time microsec
...
for t = t_pos
    ...
    set(htitle,'String',fig_title,'FontSize',18);
...
%         set(h, 'XData', p0x, 'YData', zp0, 'ZData', p0y, 'Linewidth', 2.5, 'Color', 'b')
        ...
        %         im=frame2im(frame);
        %         [imind,cmap]=rgb2ind(im,256);
        %         imwrite(imind,cmap,['gifs\',num2str(shotnum), '_mag.gif'],'gif','WriteMode','append','DelayTime',delay);
    end
...
%         set(mm, 'XData', real(cx(:,1)), 'YData', zm(:,1), 'ZData', real(cy(:,1)), 'Linewidth', 1.5, 'Color', color(1), 'LineStyle', '-')
    ...
    %     i = 87;
    %     yMDS = [data{i}(t),data{i+1}(t),data{i+2}(t),data{i+3}(t),data{i+4}(t),data{i+5}(t),data{i+6}(t),data{i+7}(t),data{i+8}(t),data{i+9}(t)];
    %
    %     figure(4)
    %     plot(z0,100*yMDS, 'Linewidth', 2)
    %     xlabel('z [cm]')
    %     ylabel('Displacement [cm]')
    %     title('Vertical component of m1 mode data at time t')
    %     daspect([2 1 1])
    %     axis([0 45 -2.13/2*2.54 2.13/2*2.54]);
    %     ax=gca;
    %     set(ax,'XTick',[0,5,10,15,20,25,30,35,40,45]);
    %     set(ax,'YTick',[-2.7,-2,-1,0,1,2,2.7]);
    ...
    % ======================================================================
    % Plot - Contour quiescent criteria
    % ======================================================================
    
    %     set(hcq, 'XData', z0, 'YData', t_contour, 'ZData', Bcq,'LevelList', rangeq, 'LineWidth', 2)
    %     drawnow
    
    %         frame=getframe(5);
    %         im=frame2im(frame);
    %         [imind,cmap]=rgb2ind(im,256);
    %         imwrite(imind,cmap,['gifs\',num2str(shotnum), '_m1m0cont.gif'],'gif','WriteMode','append','DelayTime',delay);
    % pause
...
figure(11)
surf(T(:,1:end-2),Z(:,1:end-2),real(u_dmd))
xlabel('t')
ylabel('x')
title('DMD generated data')
figure(12)
surf(T(:,1:end-2),Z(:,1:end-2),real(u_dmd)-b_th180(:,1:end-2))
xlabel('t')
ylabel('x')
title('Error')
